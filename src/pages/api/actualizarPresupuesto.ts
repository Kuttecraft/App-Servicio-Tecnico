...
export async function POST(context: { request: Request }) {
  ...
  // Upsert presupuesto (última fila como verdad)
  ...
  // Actualizar preferencia tri-estado en el ticket
  if ('solicitar_presupuesto' in fields) {
    const { error: updPrefErr } = await supabase
      ...
      .eq('id', ticketIdNum);
    if (updPrefErr) {
      return new Response(JSON.stringify({ error: 'No se pudo guardar la preferencia: ' + updPrefErr.message }), {
        status: 500, headers: { 'Content-Type': 'application/json' },
      });
    }
  }

  // Mantener estado = "P. Enviado"
